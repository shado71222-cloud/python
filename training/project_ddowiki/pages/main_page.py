...
class MainPage():
    ...
    def equipment_slot_check(self):
        try:
            slot_finder = self.page.locator(".mw-redirect").locator("xpath=ancestor::tr").filter(
                has=self.page.locator("[title='Slot']"))
    ...
    def plat_per_weight(self, search):
        try:
            raw_plat_finder = self.page.locator(".bg-color-1").locator("xpath=ancestor::tr").filter(
                has_text="Base Value")
