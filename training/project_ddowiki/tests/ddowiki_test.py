from training.project_ddowiki.globals import search_global
...
# some tests are using try and except in case the search is faulty so it doesn't crush the rest of the tests
...
class Test_ddowiki:
    ...
    def test_basic_search(self, setup_playwright_project_wiki):
        ...
        # the result should be "d_" where the _ is the number
    ...
    def test_return_to_home_page(self, setup_playwright_project_wiki):
        ...
        # example for items that will fail:sword_of_shadow

    def test_equipment_slot_check(self, setup_playwright_project_wiki):
        # search = "cloak_of_the_Zephyr"  #moved to global here just in case
        page = setup_playwright_project_wiki
        main_page = MainPage(page)
        main_page.search_bar(search_global)
    ...
    def test_who_gets_more_hitpoints(self, setup_playwright_project_wiki):
        ...
        assert int(first_class_result) > int(
            second_class_result), f"{first_class} doesn't get more hitpoints than {second_class}"
